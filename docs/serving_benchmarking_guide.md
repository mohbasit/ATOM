...
| `ATOM_ENABLE_DETAILED_ANNOTATION=1` env var | Appends attention FLOP aggregates (`sqsq`, `sqsk`, `sk`) to the `prefill[]`/`decode[]` trace labels while profiling is active (see [5.6](#56-cuda-graph-capture-traces)) |
...
`ATOM_ENABLE_DETAILED_ANNOTATION=1` (see [5.1](#51-configuration)). The added
